...
local function pickInitialSpawnCFrame(): CFrame
	...
	warn("[Round] InitialSpawnBox missing \xE2\x80\x94 falling back to (0, 100, 0)")
...
local function enterAssigningTeams(system)
	...
	if not mapTemplate then
		warn(`[Round] Map "{mapName}" not found \xE2\x80\x94 aborting`)
	...
	for _, player in system._pendingPlayers do
		...
		if not team then
			...
			warn(`[Round] {player.Name} had no team \xE2\x80\x94 dynamically assigned to team {team}`)
...
local function enterRoundActive(system)
	...
	task.delay(Configs.POSITIONING_OUTER_TIMEOUT, function()
		if finalized then return end
		if not isCurrentRound() then return end
		warn("[Round] Positioning outer safety timer fired \xE2\x80\x94 force-finalizing")
		for _, player in system._roundRoster do
			...
			then
				warn(`[Round] {player.Name} was not positioned before timeout \xE2\x80\x94 forcing Skipped`)
